...
# Enhanced Streamlit UI with better visualization
def display_results(result, action):
    ...
    if isinstance(result, dict) and 'error' in result:
        ...
        st.bar_chart(pd.DataFrame.from_dict(result, orient='index'))
    else:
        st.write(result)

# Main application
def main():
    st.title("Advanced BI Analytics Workbench")
    ...
    if uploaded_file:
        ...
        query = st.text_input("Enter your analytical query:", 
                            placeholder="E.g., 'Show top 5 sales by region'")
        
        if query:
            result, action = execute_query(query, df)
            display_results(result, action)
